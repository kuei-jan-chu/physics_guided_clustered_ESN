...
with open(base_path + "/Simulation_Data/simulation_data.pickle", "rb") as file:
...
for N_plot in [1000, 10000, 100000]:
    ...
    plt.savefig(base_path + "/Figures/Plot_U_first_N{:d}.pdf".format(N_plot), dpi = 300, bbox_inches="tight")
    plt.close()
...
for N_plot in [1000, 10000, 100000]:
    ...
    plt.xlabel(r"$x$")
    plt.ylabel(r"$n, \quad t=n \cdot {:}$".format(dt))
    plt.savefig(base_path + "/Figures/Plot_U_last_N{:d}.pdf".format(N_plot), dpi = 300, bbox_inches="tight")
    plt.close()
