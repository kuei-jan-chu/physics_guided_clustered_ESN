...
--parse_string "round_(\d+)_grid_search_include_density-(.+?)_reservoir_size_(\d+)_NTrain_100000_Noise_5_Regularization_(\d+\.\d+)_step_size_1" \
--x_value "Reservoir size" \
--x_value_group_num 3 \
--specific_model_type $SPECIFIC_MODEL_TYPE \
--parse_string_for_specific_model  "round_(\d+)_grid-search_for_extracted_knowledge_from_(\d+)-auto_pgclustered_esn_(.+?)_reservoir_size_(\d+)_NTrain_100000_Noise_5_Regularization_(\d+\.\d+)_step_size_1" \
