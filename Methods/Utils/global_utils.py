...
def getModel(params_dict):
	...
	elif params_dict["model_name"] == "auto_pgclustered_esn":
		import auto_pgclustered_esn as model
		return model.AutoPGClusteredESN(params_dict)
...
def trainParalleledModelMultipleTimes(params_dict):
	...
	for i in range(trial_num):
		...
		train_command = [
			...
			"--step_size", str(params_dict["step_size"]),
		]
		try:
...
def averageResultOfModelMultipleTimes(params_dict, exp_name):
	...
	data_path = params_dict["saving_path"] + params_dict["results_dir"] + "round_{:}_{:}_system_name_{:}_reservoir_size_{:}_NTrain_{:}_Noise_{:}_Regularization_{:}_step_size_{:}".format(params_dict["hyper_tuning_round_num"], exp_name, \
																						params_dict["system_name"], params_dict["approx_reservoir_size"], params_dict["N_train"], params_dict["noise_level"], params_dict["regularization"], params_dict["step_size"]) + "_best_averaged_result.pickle"
...
def getAutoPGClusteredESNParser(parser):
	parser = getStandardESNParser(parser)
	# parser.add_argument("--corresponding_input_weight", help="control the input strength of the corresponding input dimension for each cluster in the reservoir", type=float, required=False, default=0.8)
	parser.add_argument("--input_group_size", help="the size of dimensions of input for each cluster", type=int, required=False, default=1)
	parser.add_argument("--coupling_from_length", help="the length used to extract coupling knowledge", type=int, required=True)
	return parser
