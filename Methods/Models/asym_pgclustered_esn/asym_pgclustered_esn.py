...
class AsymPGClusteredESN(ESN):
    def __init__(self, params):
        super().__init__(params)    

        # Parameters specific to PGClusteredESN
        self.in_cluster_weight = params["in_cluster_weight"]
        # self.corresponding_input_weight = params["corresponding_input_weight"]
        self.coupling_dims = [list(map(int, group.split())) for group in params["coupling_dims"].split(';')]
    ...
    def getReservoirWeights(self, size_x, size_y, radius, sparsity):
        ...
        for i in range(self.num_clusters):
            ...
            for j in self.coupling_dims[i]:
                coupled_cluster_start_idx = ((i + j) % self.num_clusters) * cluster_size
