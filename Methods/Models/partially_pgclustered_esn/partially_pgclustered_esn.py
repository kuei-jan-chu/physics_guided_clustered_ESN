...
class PartPGClusteredESN(ESN):
    def __init__(self, params):
        ...
        self.prob_wrong_couplings = params["prob_wrong_couplings"]
    ...
    def getReservoirWeights(self, size_x, size_y, radius, sparsity):
        ...
        for i in range(num_clusters):
            ...
            # generate dimension choices for wrong coupling 
            if num_clusters % 2 == 0:
                wrong_coupling_dims = [i for i in range(-int((num_clusters)/2), int((num_clusters)/2)) if i != 0 and i not in self.coupling_dims]
            else:
                wrong_coupling_dims = [i for i in range(-int((num_clusters)/2), int((num_clusters)/2)+1) if i != 0 and i not in self.coupling_dims]

            # Connect to the coupled clusters
            for j in self.coupling_dims:
                wrong_coupling = torch.rand(1)
                if wrong_coupling < self.prob_wrong_couplings and len(wrong_coupling_dims) > 0:
                    random_index = torch.randint(0, len(wrong_coupling_dims), (1,))
                    j = wrong_coupling_dims[random_index]
                    wrong_coupling_dims.pop(random_index)
