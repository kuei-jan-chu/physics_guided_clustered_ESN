...
class AutoPGClusteredESN(ESN):
    def __init__(self, params):
        super().__init__(params)    

        self.coupling_from_length = params["coupling_from_length"]
        ...
        self.coupling_dims = None
        self.getCouplingDims()
    ...
    def getKeysInModelName(self):
        ...
        'regularization':'REG',
        'coupling_from_length':'CFL',
        'worker_id':'WID', 
        }
        return keys


    def getCouplingDims(self):
        with open(self.train_data_path, "rb") as file:
            # Pickle the "data" dictionary using the highest protocol available.
            data = pickle.load(file)
            TE_matrix = data["transfer_entropy_matrix_from_length_{:}".format(self.coupling_from_length)]
        
        coupling_num = int(self.num_clusters * self.sparsity) - 1  # -1 because we don't count self coupling
        coupling_dims = np.zeros((self.num_clusters, coupling_num), dtype=int)
        for i in range(self.num_clusters):
            coupling_dims[i, :] = np.argsort(TE_matrix[i, :])[-coupling_num:]
        coupling_dims = np.sort(coupling_dims, axis=1)

        self.coupling_dims = coupling_dims
        # print("Coupling dimensions for each cluster: ", self.coupling_dims)
    ...
    def getReservoirWeights(self, size_x, size_y, radius, sparsity):
        ...
        for i in range(num_clusters):
            ...
            for j in self.coupling_dims[i]:
                coupled_cluster_start_idx = j * cluster_size
        ...
        # data_path = self.saving_path +self.results_dir +self.model_name + "/reservoir_layer_weight.txt"
        # os.makedirs(self.saving_path + self.results_dir + self.model_name, exist_ok=True)
        # np.savetxt(data_path, W_h.cpu().numpy(),fmt="%.6f", delimiter=",")
    ...
    def getInputLayerWeights(self, reservoir_size, input_dim, sigma_input, sparsity):
        ...
        for i in range(num_clusters):
            ...
            for j in self.coupling_dims[i]:
                coupled_start_input_dim = j * input_group_size
        ...
        # data_path = self.saving_path +self.results_dir +self.model_name + "/input_layer_weight.txt"
        # os.makedirs(self.saving_path + self.results_dir + self.model_name, exist_ok=True)
        # np.savetxt(data_path, W_in.cpu().numpy(),fmt="%.6f", delimiter=",")
